...
import com.trimble.agmantra.layers.CPoint;
...
public class Mercator {
    ...
    public static long latToY(double dLat) {
        ...
        double dY = Math.toDegrees(Math.log(Math.tan((Math.PI / 4) + (radianLat / 2))))
                * dDegreesToMetersFactor;

        if (dY > lMaxMeters) {
            System.out.println("Input Lat " + dLat + " exceeds limit. Using limit instead.");
            return sign * lMaxMeters;
        }

        dY = Math.round(sign * dY);

        dY = 1050000000L - dY * 50.0;
        return (long)Round(dY, 0);
    }
    ...
    public static long lonToX(double dLon) {
        ...
        if (dX > lMaxMeters) {
            System.out.println("Input Lon " + dLon + " exceeds limit. Using limit instead.");
            return lMaxMeters;
        } else if (dX < lMinMeters) {
            System.out.println("Input Lon " + dLon + " exceeds limit. Using limit instead.");
            return lMinMeters;
        }
        dX = Math.round(dX);

        dX = dX * 50.0 + 1050000000L;
        return (long)Round(dX, 0);
    }

    // X in meter Lon
    public static double xToLon(long dX) {
        double dLon = (dX - 1050000000L) / 50.0;
        dLon = dLon / dDegreesToMetersFactor;
        return dLon;
    }

    // Y in meter Lat
    public static double yToLat(long dY) {
        double dLat = ((1050000000L - dY) / 50.0);
        double radianY = Math.toRadians(dLat / dDegreesToMetersFactor);
        dLat = Math.toDegrees(Math.atan(Math.sinh(radianY)));
        return dLat;
    }
    ...
    public static int MoveNorth (CPoint mPoint, double Dist) {
      ...
      public static CPoint pixelTometer(int iXinPixel, int iYinPixel, int iZoom,
            int iCenterX, int iCenterY, int iMapWidth, int iMapHeight,
            int iOrientation) {
