package com.trimble.ag.filemonitor.utils;
...
// DateFormat.getDateTimeInstance(DateFormat.MEDIUM,DateFormat.MEDIUM);
   ...
   public static void hideSoftKeyboard(Context context, EditText searchBox) {
	   ...
	   if(context != null && searchBox != null){
   ...
   public static String getHoursMins(long time) {
      ...
      final String stTime = hours + ":" + minutes ;
      return stTime;
...
// .append(ST_NEWLINE);
   ...
   public  static String getDoubleWithOutComma(String stData){
        if(stData == null || stData.trim().length() == 0){
                return stData;
        }
           if(stData.indexOf(COMMA) != -1){
   ...
   public  static String getFormatDataBy(String stData){
           ...
           String stDummy=getDecimalFormatSymbols();
                boolean isDotPersent=stDummy.equals(DOT);
                boolean isCommaPersent=stDummy.contains(COMMA);
                        if(isDotPersent){
                        stData=getFormatDataBy(stData,COMMA,DOT);
                        }else if(isCommaPersent){
                        stData=getFormatDataBy(stData,DOT,COMMA);
           }
    ...
    public static String getNumberFormatForCurrentLocale(final double dData,final boolean isGroupRequired){
       String stData=null;
       final NumberFormat nf = NumberFormat.getInstance();
           nf.setGroupingUsed(isGroupRequired);
           stData=nf.format(dData);
       return stData;
    }
