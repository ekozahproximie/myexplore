...
public class MapActivity extends Activity implements OpenStreetMapConstants {
	...
	public void onCreate(final Bundle savedInstanceState) {
		...
		ALKMapTileSource.retrieveALKKey(getApplicationContext());
		MapboxTileSource.getKeys(getApplicationContext());
		...
		final String tileSourceName = mPrefs.getString(PREFS_TILE_SOURCE,
				TileSourceFactory.ALKROADMAPTILESOURCE.name());

		final ITileSource tileSource = TileSourceFactory
				.getTileSource(tileSourceName);
		...
		/*
		 * This is an example of usage of runOnFirstFix. It looks more
		 * complicated than necessary because we need to create an extra thread
		 * and a handler. If you wanted to do a non-GUI thread then you wouldn't
		 * need the handler.
		 */
		if (DEBUGMODE) {
	...
	protected void onPause() {
		...
		edit.putString(PREFS_TILE_SOURCE, mOsmv.getTileProvider()
				.getTileSource().name());
		...
		final ITileSource tileSource = TileSourceFactory.getTileSource(mOsmv
				.getTileProvider().getTileSource().name());
	...
	private void addWeoGeoLabel(){
	   mOsmv. insertSecondaryTileOverlayToBottom(getBaseContext(), TileSourceFactory
                 .getTileSource("WeoGeo"), 255, 1);
	}
	@Override
	protected void onResume() {
		...
		try {
			...
			Log.i("test", "tileSourceName:"+tileSourceName);
			mOsmv.setTileSource(tileSource);
			if(tileSourceName.equals("HWeoGeo")){
			   addWeoGeoLabel();
	                }
			   
			 //  Read more: http://datamoil.blogspot.com/2011/05/offline-google-maps-on-osmdroid.html#ixzz3UFPYl6P6
	...
	public boolean onCreateOptionsMenu(final Menu pMenu) {
	    ...
	    // Put overlay items next
           mOsmv.getOverlayManager().onCreateOptionsMenu(pMenu, MENU_LAST_ID,
                           mOsmv);
	...
	public boolean onPrepareOptionsMenu(final Menu pMenu) {
		mOsmv.getOverlayManager().onPrepareOptionsMenu(pMenu, MENU_LAST_ID,
				mOsmv);
	...
	public boolean onMenuItemSelected(final int featureId, final MenuItem item) {
		...
		default:{
			final boolean returnValue= mOsmv.getOverlayManager().onOptionsItemSelected(item,
					MENU_LAST_ID, mOsmv);
			if(item.getItemId() == 9){
			   addWeoGeoLabel();
			}else{
			   
			   if(mOsmv.getSecondaryOverlay() != null){
			      mOsmv.removeSecondaryTileOverlay(mOsmv.getSecondaryOverlay());   
			   }
