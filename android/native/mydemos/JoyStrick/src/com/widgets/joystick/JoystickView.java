...
public class JoystickView extends View {
	...
	protected void onDraw(Canvas canvas) {
		...
		if (D) {
		  ...
		  if(movementConstraint == CONSTRAIN_CIRCLE && isMove){
              int a =cX ,b=cY,r =movementRadius;
              double an=angle* 57.2957795d;
             
              double t = /*2 * Math.PI**/Math.toRadians(an);// * (1 / 4);
              ...
              //Log.d(TAG, "angle :"+an);
              //Log.d(TAG,"angle "+(int)Math.round((  ((double)(an) % 360) / 60)));
              
              canvas.drawCircle(x, y, 8, handlePaint);
          }
...
//				Log.d(TAG, String.format("moveListener.OnMoved(%d,%d)", (int)userX, (int)userY));
				moveListener.OnMoved(userX, userY,angle* 57.2957795d);
			}
		}
	}
